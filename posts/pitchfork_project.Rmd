...
The reason for this approach was, in a sense, to assume nothing about the genres to begin with -- to let the data decide which genres the reviews should be split into. This also means that, for the remaining genres, there are hopefully enough exemplars in the database from which to draw meaningful conclusions, *i.e.* we wwon't end up trying to comment upon how a genre is reviewed, based on only one or two albums.
